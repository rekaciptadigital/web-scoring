...
function useScheduleEditorForm(initialValues, eventId) {
  ...
  const submitSchedules = ({ onSuccess }) => {
    const payload = makePayloadScheduleForm(eventId, state.data);
...
function makePayloadScheduleForm(eventId, editorFormdata) {
  return {
    event_id: eventId,
    qualificationTime: editorFormdata.sessions.map((session) => {
